...
#                       replace = TRUE)
#     model@X <- as.matrix(model@X[indices] + rnorm(length(indices),
#                                                   0, 0.05))
#     model@y <- as.matrix(model@y[indices] + rnorm(length(indices),
#                                                   0, 0.05))
...
funs_to_test = funs_to_test[[1]] # only alpine 2
...
iters <- 20
init_design <- 4
lambdas = c(0.1,0.5,1,2,3,5,7,10)
...
for (lambda in lambdas) {
  ...
  ctrl <- setMBOControlInfill(ctrl, crit = makeMBOInfillCritCB(cb.lambda = lambda), opt = "focussearch",
  ...
  for (test_run in 1:n) {
    ...
    tryCatch({
      ...
      assign("BO_failed", BO_failed, envir = globalenv())
    })
  ...
  index_lambda = which(lambdas == lambda)
  global_res_weightedML[[index_lambda]] = opt_path_best_y_w
  global_res_unweightedML[[index_lambda]] = opt_path_best_y_uw
...
for (i in 1:length(lambdas)) {
  BO_paths_all_means <- matrix(nrow = iters, ncol = length(lambdas))
...
save(w_paths_mop, file = "weighted-ML-GP/results/weighted-ML-results-mop-n-100-iters-25-ninit-4-alpine-2-lcb-sampling-based-low-vals")
save(w_paths_mop_sd, file = "weighted-ML-GP/results/weighted-ML-results-mop-sd-n-100-iters-25-ninit-4-funs-alpine-2-lcb-sampling-based-low-vals")
save(uw_paths_mop, file = "weighted-ML-GP/results/unweighted-ML-results-mop-n-100-iters-25-ninit-4-funs-alpine-2-lcb-sampling-based-low-vals")
save(uw_paths_mop_sd, file = "weighted-ML-GP/results/unweighted-ML-results-mop-sd-n-100-iters-25-ninit-4-funs-alpine-2-lcb-sampling-based-low-vals")

# save(global_res_weightedML, file = "weighted-ML-GP/results/weighted-ML-results-global-n-100-iters-20-ninit-4-funs-3-4-6-8-27-31-sampling-based")
# save(global_res_unweightedML, file = "weighted-ML-GP/results/weighted-ML-results-global-n-100-iters-20-ninit-4-funs-3-4-6-8-27-31-sampling-based")
