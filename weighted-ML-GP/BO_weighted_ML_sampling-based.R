BO_weighted_ML <- function(fun = NULL,
  ...
  for (i in 1:global_iters) {
    assign("i", i, envir = globalenv()) # needed in edited version of loglikfun
  ...
  logLikFun <-
    function(param, model, envir=NULL) {
      ...
      if(nrow(model@X) == length(weights)){
        # resample data with weights
        indices <- sample(1:length(weights), prob = weights, replace = TRUE)
        model@X <- model@X[indices]
        model@y <- model@y[indices]
      }
#stop("hi")      #warning(list(weights))
      ## end edit
...
back_to_normal_LogLikFun <- function() {
  ...
  # this file overwries the function logLikFun() in DiceKriging
  # WARNING: Only source for specific purposes
