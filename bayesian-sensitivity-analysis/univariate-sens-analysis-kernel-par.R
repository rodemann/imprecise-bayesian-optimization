...
# ground truth to get true mean for each function: 
...
for (fun in sequence_functions) {
  ...
  baseline_km <- km(design = ground_truth, response = y_true, 
                    nugget = nugget_true, covtype = "gauss",  control = list(trace = FALSE) )
...
BO_paths_acc_difference_kernel_par <- BO_paths_acc_difference
BO_paths_acc_difference_kernel_par
save(BO_paths_acc_difference_kernel_par, file = "bayesian-sensitivity-analysis/univariate-acc-diff-kernel-par")
