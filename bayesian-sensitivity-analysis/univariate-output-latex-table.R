...
## standardization
acc_diff_df_plain <-acc_diff_df_1d[,-1]
mean_acc_diff <- apply(acc_diff_df_plain, 1, mean)
acc_diff_df_standardized <- acc_diff_df_1d %>% mutate("Mean Functional Form stand." = Mean.Functional.Form/mean_acc_diff ) %>% mutate("Mean Parameters stand." = Mean.Parameters/mean_acc_diff ) %>% mutate("Kernel Functional Form stand." = Kernel.Functional.Form/mean_acc_diff ) %>% mutate("Kernel Parameters stand." = Kernel.Parameters/mean_acc_diff )
