...
library(smoof)
library(viridis)
source("mean-fun-params-analysis/draw_plot.R")
source("mean-fun-params-analysis/draw_legend_mean.R")
...
#  create legend
names <- c("m(x) = -140000","m(x) = 56000", "m(x) = 70000", 
           "m(x) = 84000", "m(x) = 280000")
names <- c("m(x) = -40", "m(x) = 16", "m(x) = 20", "m(x) =  24", "m(x) =  80")
draw_legend_mean(pal, names)

# load results to environment
load("mean-fun-params-analysis/BO_results_40_20_meanfun")
...
geom_hline(yintercept = global.opt, linetype = "dashed",  color = optimum_col) + 
  labs(title = paste("Bayesian Optimization of", fun_name), 
       subtitle = "40 BO runs per Mean with 20 iterations each. 
...
#plot(obj.fun[[1]])
#plot3D(obj.fun[[1]])
#plot3D(obj.fun.list[[8]], package = "plotly")
