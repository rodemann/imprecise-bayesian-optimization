...
library(smoof)
library(viridis)
source("mean-fun-trend-analysis/draw_plot.R")
source("mean-fun-trend-analysis/draw_legend_mean.R")
...
#  create leegend
names <- c("constant","linear", "quadratic", "cubic")
draw_legend_mean(pal, names)

# load results to environment
load("mean-fun-trend-analysis/BO_results_60_20_meanfun_trend")
# select function
plots = list()
...
plot = draw_plot(fun = f, mbo_results_paths = mbo_results_paths,
          initial_design_size = 10, pal, configs = 4) +
...
geom_hline(yintercept = global.opt, linetype = "dashed",  color = optimum_col) + 
  labs(title = paste("Bayesian Optimization of", fun_name), 
       subtitle = "40 BO runs per Mean Function with 20 iterations each. 
