...
`kmNuggets.init` <-
  function(model) {
    ...
    # begin edit (jr)
    if(length(y.dist[I]) == 0)
      varinit.vario.total <- 0
    if(length(nugget.aux.I) == 0)
      nugget.aux.I <- 0
    #end edit (jr)
...
environment(kmNuggets.init) <- asNamespace('DiceKriging')
assignInNamespace("kmNuggets.init", kmNuggets.init, "DiceKriging")
