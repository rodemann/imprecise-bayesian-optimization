...
base_kernel = "powexp"
lrn = makeLearner("igp_upper_mean", base_kernel = base_kernel, imprecision = 30, predict.type = "se")
mod_upper = train(lrn, .task)
lrn = makeLearner("igp_lower_mean", base_kernel = base_kernel, imprecision = 30, predict.type = "se")
...
preds_mean_lower = preds$data$response
#par(bg = '40gray')
plot(x = grid , y = preds_mean_lower, type = "l", col = "darkblue", ylab = "Alpine Function f(x)", xlab = "x", ylim = c(-10,15))
# 95 % CIs
preds_lower_up <- preds$data$response + qnorm(0.975) * preds$data$se
lines(x = grid, y = preds_lower_up, lty = 3, col = "darkblue")
preds_lower_low <- preds$data$response - qnorm(0.975) * preds$data$se
lines(x = grid, y = preds_lower_low, lty = 3, col = "darkblue")

#upper
preds = predict(mod_upper, newdata = test_data)
preds_mean_upper = preds$data$response
lines(x = grid , y = preds_mean_upper, type = "l", col = "red")
# 95 % CIs
preds_upper_up <- preds$data$response + qnorm(0.975) * preds$data$se
lines(x = grid, y = preds_upper_up, lty = 3, col = "red")
preds_upper_low <- preds$data$response - qnorm(0.975) * preds$data$se
lines(x = grid, y = preds_upper_low, lty = 3, col = "red")
...
lines(pred_grid_mean, col = "black")
# CIs from classic GP
pred_grid_lower = cbind(test_points, precise_GP$lower95)
pred_grid_upper = cbind(test_points, precise_GP$upper95)
lines(pred_grid_upper, lty = 3, col= "black")
lines(pred_grid_lower, , lty = 3, col = "black")


#grid(col = "lightgray", lty = "longdash")

legend("top", legend=c("IGP Upper Mean", "Precise GP", "IGP Lower Mean", "0.95 Credible Intervalls"),
       col=c("red", "black", "darkblue", "black"), lty=c(1,1,1,2) , cex=0.8)



# now turn it into a nice ggplot
# df = data.frame("x" = test_points, preds_upper_up, "IGP Upper mean" = preds_mean_upper, preds_upper_low, 
#                 pred_grid_upper[,2], pred_grid_mean[,2], pred_grid_lower[,2],
#                 preds_lower_up, preds_mean_lower, preds_lower_low)
# 
