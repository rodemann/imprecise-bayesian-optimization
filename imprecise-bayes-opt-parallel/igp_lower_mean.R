...
makeRLearner.igp_lower_mean = function() {
  makeRLearnerRegr(
    cl = "igp_lower_mean",
    package = "DiceKriging",
    ...
    name = "Lower Posterior Mean of Imprecise Gaussian Process",
    short.name = "igp",
    note = ""
  )
}
...
trainLearner.igp_lower_mean = function(.learner, .task, .subset,
                                       .weights = NULL, ...) {
  args = list(...)
  ...
  too_close <- lapply(list(design, response), function(data){
    ...
    too_close_data <- which(dist_vector < 0.001) %>% unname()
    too_close_nr <- sum(dist_vector < 0.001)
  ...
  sum_all_k <- as.double(sum_all_k) # seems necessary
    ...
    igp_lower_mean_scalar <- function(scalar) {
      ...
      if(args$base_kernel == "powexp")
        ...
        # compute lower bound of mean prediction
        if (abs(t(sum_rows_k) %*% response / sum_all_k) <= 1 + args$imprecision/sum_all_k){
          lower_mean <- (t(kernel_vector) %*% Kernel_matrix_n_inverse %*% response 
        ...
        if (abs(t(sum_rows_k) %*% response / sum_all_k) > 1 + args$imprecision/sum_all_k){
          lower_mean <- (t(kernel_vector) %*% Kernel_matrix_n_inverse %*% response 
                         + (1 - t(kernel_vector) %*% sum_rows_k) %*% (t(sum_rows_k) %*% response / (sum_all_k + args$imprecision)))
        } 
        lower_mean
    }
    
    igp_lower_mean_se_scalar <- function(scalar) {
      # compute kernel vectors and matrices required for definition of standard error (std. deviation) in Theorem 4 
      # in Mangili (2015). Notation follows the theorem. Distance in kernel is 0, see k(x,x) in theorem 4
      distance <- abs(design - rep(scalar,nrow(design)))
      if(args$base_kernel == "powexp")
        kernel_vector <- base_kernel_fun(distance, theta, var = var, p = power) else
          kernel_vector <- base_kernel_fun(distance, theta, var = var)
        if(args$base_kernel == "powexp")
          kernel_null <- base_kernel_fun(0, theta, var = var, p = power) else
            kernel_null <- base_kernel_fun(0, theta, var = var)
          #compute standard error  
          lower_mean_var <- (kernel_null - (t(kernel_vector) %*% Kernel_matrix_n_inverse %*% as.matrix(kernel_vector)) 
                             + ((1 - t(kernel_vector) %*% sum_rows_k)^2 / sum_all_k))
          # control for tiny negative variances due to numerical instability
          #if(-0.00001 <= lower_mean_var & lower_mean_var < 0) lower_mean_var <- 0
          if(lower_mean_var < 0) stop(paste("negative var estimates produced by igp lower mean of value", lower_mean_var))
          lower_mean_se <- sqrt(lower_mean_var)
          lower_mean_se
    }
  ...
  igp_lower_mean <- function(x){  
    ...
    if(.learner$predict.type == "response"){
      pred$mean <- apply(x, 1, igp_lower_mean_scalar)
    }
    if(.learner$predict.type == "se"){
      pred$mean <- apply(x, 1, igp_lower_mean_scalar)
      pred$se <- apply(x, 1, igp_lower_mean_se_scalar)
    }
    pred
  } 
  
  
  model = structure(list(x = design, y = response), class = "igp_lower_mean") 
  
  predict.igp_lower_mean = function(newdata) {
    igp_lower_mean(newdata)
  }
  model$learner.model = predict.igp_lower_mean
  return(model)
}    


# (3) mlr predict function
predictLearner.igp_lower_mean = function(.learner, .model, .newdata, ...) {
