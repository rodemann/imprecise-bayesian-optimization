...
library(smoof)
library(viridis)
source("kernel-analysis/draw_plot.R")
source("kernel-analysis/draw_legend.R")
...
geom_hline(yintercept = global.opt, linetype = "dashed",  color = optimum_col) + 
  labs(title = paste("Bayesian Optimization of", fun_name), 
       subtitle = "60 BO runs per Kernel Function with 20 iterations each. 
